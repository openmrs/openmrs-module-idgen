...
public class RemoteWithLocalPoolIntegrationTest extends
	...
	public void testConfigurePoolFilledFromRemoteSource() throws Exception {
		...
		IdentifierSourceService service = Context
				.getService(IdentifierSourceService.class);

		// register a stub RemoteIdentifierSourceProcessor that won't really go
		// to the internet
		...
		service.registerProcessor(RemoteIdentifierSource.class,
				remoteProcessorStub);

		PatientIdentifierType socialSecurityNumber = Context
				.getPatientService().getPatientIdentifierType(
						SOCIAL_SECURITY_NUMBER_PATIENT_IDENTIFIER_TYPE);
		...
		remoteSource
				.setUrl("http://urlforremotesource/openmrs/idgen/exportIdentifiers.form?source=1&comment=Mirebalais");
		...
		// the first time we request an identifier it should make 2 remote
		// requests with batchSize=3, and then give us one of those
		Assert.assertEquals("1",
				service.generateIdentifier(socialSecurityNumber, "First"));
		Assert.assertEquals("Pool should have 5 available", 5,
				service.getQuantityInPool(pool, true, false));
		Assert.assertEquals("Pool should have 1 used", 1,
				service.getQuantityInPool(pool, false, true));
		Assert.assertEquals(2, remoteProcessorStub.getTimesCalled());

		// the next two requests should not make remote requests
		Assert.assertEquals("2",
				service.generateIdentifier(socialSecurityNumber, "Second"));
		Assert.assertEquals("Pool should have 4 available", 4,
				service.getQuantityInPool(pool, true, false));
		Assert.assertEquals("Pool should have 2 used", 2,
				service.getQuantityInPool(pool, false, true));
		Assert.assertEquals("3",
				service.generateIdentifier(socialSecurityNumber, "Third"));
		Assert.assertEquals("Pool should have 3 available", 3,
				service.getQuantityInPool(pool, true, false));
		Assert.assertEquals("Pool should have 3 used", 3,
				service.getQuantityInPool(pool, false, true));
		Assert.assertEquals(2, remoteProcessorStub.getTimesCalled());

		// since we're below our min pool size, the next request will make a
		// remote request
		Assert.assertEquals("4",
				service.generateIdentifier(socialSecurityNumber, "Fourth"));
		Assert.assertEquals("Pool should have 5 available", 5,
				service.getQuantityInPool(pool, true, false));
		Assert.assertEquals("Pool should have 4 used", 4,
				service.getQuantityInPool(pool, false, true));
	 ...
	 *
	 */
	public class RemoteIdentifierSourceProcessorStub extends
			RemoteIdentifierSourceProcessor {
