...
<script type="text/javascript">
	function addIdentifier(id, type, location, pref, oldIdentifier) {
		if (type != null && type != '') {
			...
			$("#newIdentifierTypeSelector").val('');
	
			var nameCell = getElementsByClass(newrow, 'identifierTypeName', 'td');
			<openmrs:forEachRecord name="patientIdentifierType">
				if (type == '${record.patientIdentifierTypeId}') {
					$(nameCell).append("${record.name}");
					var identifierTypeHidden = getElementsByClass(newrow, 'identifierTypeHidden', 'input');
					$(identifierTypeHidden).val('${record.patientIdentifierTypeId}');
	...
	function identifierOrTypeChanged(input) {
		...
		for (var i in inputs) {
			if (inputs[i] && inputs[i].name == "preferred")
				...
				idInput = inputs[i];
			else if (inputs[i] && inputs[i].name == "identifierType")
				typeInput = inputs[i];
		}
	...
	function autogenClicked(element) {
		var txtNode=element.parentNode.parentNode.getElementsByTagName('input')[2];
		if (element.checked) {
		...
		<td class="inputCell">
			<table id="identifiers" cellspacing="2">
				...
				<tbody id="identifiersTbody">
					<tr id="identifierRow" style="display:none;">
						<td valign="top" class="identifierTypeName">
							<input type="hidden" class="identifierTypeHidden" name="identifierType" value=""/>
						</td>
						<c:if test="${!empty autoGenerationOptions}">
							<td valign="top" align="center">
								<span class="autoGenerationHidden">N/A</span>
								<input class="autoGenerationOption" type="checkbox" value="" onclick="autogenClicked(this)" />
							</td>
						</c:if>
						<td valign="top"><input type="text" size="20" class="identifierInput" name="identifier" onmouseup="identifierOrTypeChanged(this)" /></td>
						<td valign="top">
			...
			<select id="newIdentifierTypeSelector" onchange="addIdentifier('', this.value, '', false, false);">
